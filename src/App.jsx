...
function getInputedProducts(list, { filter, query }) {
  let preparedProducts = [...list];

  if (filter) {
    preparedProducts = preparedProducts.filter(
      product => product.user === filter,
    );
  }
  ...
  if (query) {
    preparedProducts = preparedProducts.filter((product) => {
      ...
      return preparedName.includes(preparedQuery.trim());
    });
  }
...
export const App = () => {
  ...
  const visibleProducts = getInputedProducts(products, {
    filter: selectedUser,
    query,
  });
